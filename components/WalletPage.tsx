...
const WalletPage: React.FC<WalletPageProps> = ({ cryptos, transactions, userName }) => {
  ...
  const balances = useMemo(() => {
    const b: Record<string, number> = { 'xof': 0 };
    ...
    <div className="space-y-8 animate-in fade-in slide-in-from-bottom-6 duration-700">
      ...
      <div className="glass p-10 rounded-[3rem] bg-gradient-to-br from-indigo-900/60 to-slate-900 border-indigo-500/30">
        ...
        <div className="mt-8 flex flex-wrap gap-4">
          <button onClick={() => setIsDepositModalOpen(true)} className="bg-indigo-600 hover:bg-indigo-500 px-8 py-4 rounded-2xl font-bold transition-all shadow-xl shadow-indigo-600/30 flex items-center gap-2 text-white">Dépôt Cash (Fusion Pay)</button>
        </div>
      </div>

      <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
        <div className="glass p-8 rounded-[2.5rem] space-y-6">
          <h3 className="font-bold text-2xl tracking-tight text-white">Actifs Réels</h3>
          <div className="space-y-4">
            {Object.entries(balances)
              ...
              .map((entry: [string, number]) => {
                ...
                return (
                  <div key={id} className="flex items-center justify-between p-5 rounded-3xl bg-white/5 border border-white/5 hover:border-indigo-500/30 transition-all group">
                    <div className="flex items-center gap-4">
                      <div className="w-14 h-14 rounded-2xl bg-slate-800 flex items-center justify-center text-xl font-bold" style={{ color }}>{icon}</div>
                      <div>
                        <div className="font-bold text-white text-lg">{name}</div>
                        <div className="text-xs text-slate-500 font-mono">{amount.toLocaleString()} {symbol}</div>
                      </div>
                    </div>
        ...
        <div className="glass p-8 rounded-[2.5rem] flex flex-col justify-between">
          <div>
            <h3 className="font-bold text-2xl mb-8 tracking-tight text-white">Répartition</h3>
